...
colors:
  ...
  # ── hard · 本次新增 ──
  hard-primary:      '#000000'
  hard-background:   '#CCF224'   # 落地页底。看图估的
  hard-canvas:       '#EAF2C8'   # 白板画布底。同色系退到纸面亮度。
                                 # 依据是用户原话「会议室背景太绿了」——
                                 # 十秒的第一印象和盯一小时的工作面不是一回事。我们定的
...
**已选定并上线。** 在三版样张（贴着做 / 取其神 / 反着来）里选中的是**贴着做**，也就是本文档描述的 hard 皮肤。用户原话：「硬派皮肤的落地页可以，但会议室背景太绿了」——所以落地页保留满屏 `#CCF224`，白板画布退到 `#EAF2C8`。
...
- ✅ 落地页的酸绿可以满屏（十秒的第一印象），白板画布不行（要盯一小时）。这两处底色是两个 token，不要合并回一个。
- ✅ 改 hover 态之前先量对比度。`--c-ink-pure` 的黑底撞上顶栏里被改成黑色的 `--c-on-dark`，实测出现过按钮整个消失。

### Don'ts

> 逐字保留用户原话，不润色。这一栏只能由人填。

- ❌ **「会议室背景太绿了」** —— 看硬派的内页白板时说的。落地页那片满屏酸绿他说「可以」，画布不行。修法：画布底色单独一档，退到 `#EAF2C8`，不是把酸绿整个拿掉。

### 还没有原话的地方

Don'ts 目前只有上面这一条。第 3 问（用户在什么状态下打开）还没答，所以下面这条仍然只是**推断，不是原话**：

- ❌ 不要把酸绿做成半屏、渐变、描边或小面积强调。轴就是「保留多少」，折中会让它既不硬也不静。**标注为推断**，在用户确认之前不具备最高约束力。
...
第 1 条已定，第 2 条收到一条原话，第 3 条还空着。

1. ~~**第一次打开的人，默认看到哪一套？**~~ **已定：classic。** 硬派手动切，选择记在本机。
2. **硬派里还有什么是你一眼就不想要的？** 已经收到一条——「会议室背景太绿了」，已改。除此之外 Don'ts 还是空的：投影的轻重、洋红的调性、等宽字排中文，这些都还没听到你的说法。
3. **谁会在什么状态下打开它？** 什么时间、什么设备、单手还是双手、周围有没有人、情绪是松是紧。这一问决定 Overview 的第一句——现在那一句只写完了产品事实，缺使用状态这一半。

另外一条已经按推断实现、但你没明确说过的：**主题存本机、不随会议间同步**（同一块板上两个人可以看到不同皮肤）。依据是「主题是外观、便签是数据」。不同意就说，改起来不难。

---

*本文档由 `liebin` skill 的确认流程产出。样张：`design/liebin-2026-08-25/proof.html`（三变体 + 基线），上一版两列的在 `preview.html`。参考：stylekit neo-brutalist showcase（只学版式与气质，未使用其 logo、字体文件、图片或成段文案）。现状值全部量自 `src/index.css` 与组件源码。*
